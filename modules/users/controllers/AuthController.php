...
use app\modules\users\models\forms\ChangeOwnPasswordForm;
use app\modules\users\models\forms\ConfirmEmailForm;
use app\modules\users\models\forms\LoginForm;
use app\modules\users\models\forms\RegistrationForm;
use app\modules\users\models\forms\PasswordRecoveryForm;
...
class AuthController extends \app\modules\core\web\Controller
{
    ...
    {
        if (Yii::$app->getModule('users')->useEmailAsLogin AND Yii::$app->getModule('users')->emailConfirmationRequired) {
            ...
            throw new NotFoundHttpException(UsersModule::t('front', 'Token not found. It may be expired'));
        }
    }
        ...
        if ($model->load(Yii::$app->request->post()) AND $model->validate()) {
            if ($this->triggerModuleEvent(UserAuthEvent::BEFORE_PASSWORD_RECOVERY_REQUEST, ['model' => $model])) {
                if ($model->sendEmail(false)) {
                    ...
                    Yii::$app->session->setFlash('error', UsersModule::t('front', "Unable to send message for email provided"));
                }
            }
        }
    ...
    public function actionPasswordRecoveryReceive($token)
    {
        ...
        if (!$user) {
            throw new NotFoundHttpException(UsersModule::t('front', 'Token not found. It may be expired. Try reset password once more'));
        }
